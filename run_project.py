...
class ProjectRunner:
    ...
    def _daat_and_skip(self, postings_dict, skip_postings_dict, skip, score,queryterms):
        ...
        if len(sorted_postings) > 0:
            postings1 = self._get_postings_linked_list(sorted_postings[0][0])
            #print(postings1)
            ...
            for i in range(1, len(sorted_postings)):
                results = []
                postings2 = self._get_postings_linked_list(sorted_postings[i][0])
                #print(skip_postings2)
                j, k, skip_index1, skip_index2 = 0, 0, 0 , 0
                n1, n2 = None, None
                if postings1:
                    n1 = postings1.start_node
                if postings2:
                    n2 = postings2.start_node
                while n1 and n2:
                    if n1.value < n2.value:
                        comparisons += 1
                        if n1.skipConn and n1.skipConn <= n2.value:
                            skip_value = n1.skipConn
                            while n1 and n1.value != skip_value:
                                n1 = n1.next
                        else:
                            n1 = n1.next
                    elif n1.value > n2.value:
                        comparisons += 1
                        if n2.skipConn and n2.skipConn <= n1.value:
                            skip_value = n2.skipConn
                            while n2 and n2.value != skip_value:
                                n2 = n2.next
                        else:
                            n2 = n2.next
                    elif n1.value == n2.value:
                        comparisons += 1
                        results.append(n1.value)
                        n1 = n1.next
                        n2 = n2.next

                linkedlist = LinkedList()
                for doc in results:
                    linkedlist.insert_at_end(doc, len(results))
                linkedlist.add_skip_connections()
                postings1 = linkedlist
                i += 1
        if score:
                ...
                for query in queryterms:
                    ...
                    for doc_id in results:
                        ...
                        while node is not None and node.value != doc_id:
                            node = node.next
                        if node and node.tfidf > scores_dict[doc_id]:
    ...
    def _get_postings(self, term, skip):
        ...
        return postings
    def _get_postings_linked_list(self, term):
        """ Function to get the postings list of a term from the index.
            Use appropriate parameters & return types.
            To be implemented."""
        postings_list = None
    ...
    def run_queries(self, query_list, random_command):
        ...
        for query in tqdm(query_list):
            ...
            for term in input_term_arr:
                ...
                #print(len(postings_dict[term]))
                #print(postings)
                skip_postings_dict[term] = self._get_postings(term, True)
                #print(len(skip_postings_dict[term]))
                #print(skip_postings_dict[term])
                #print(skip_postings)
            ...
            and_op_skip = self._daat_and_skip(postings_dict, skip_postings_dict, True, False, input_term_arr)
            and_op_no_skip_sorted= self._daat_and(postings_dict, skip_postings_dict, False, True, input_term_arr)
            and_op_skip_sorted = self._daat_and_skip(postings_dict, skip_postings_dict, True, True, input_term_arr)
...
if __name__ == "__main__":
    ...
    #runner.run_queries(["the novel coronavirus pandemic"], random_command="self.indexer.get_index()['novel'].traverse_list()")
